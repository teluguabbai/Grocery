...
# COD Paymentfrom django.shortcuts import redirect
...
def cod_payment(request):
    if request.method == "POST":
        ...
        customer_address = Address.objects.filter(user=request.user).first()
        if not customer_address:
            return redirect("checkout")  # Address not found
        ...
        message = (
            ...
            f"👤 Customer: {customer_address.full_name}\n"
            f"📱 Phone: {customer_address.phone_number}\n"
            f"🏠 Address: {customer_address.street_address}, {customer_address.city}, {customer_address.state}, {customer_address.postal_code}, {customer_address.country}\n"
            f"🧾 Order ID: {order.id}\n"
        ...
        # Admin's WhatsApp number (your number)
        admin_number = "919491344068"  # Add your number with country code (e.g., +91...)
